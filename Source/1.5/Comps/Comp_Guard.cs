...
namespace aRandomKiwi.GFM
{
    public class Comp_Guard : ThingComp
    {
        ...
        public override IEnumerable<Gizmo> CompGetGizmosExtra()
        {
            ...
            if (this.parent.def.race != null && this.parent.def.race.intelligence != Intelligence.Humanlike && !this.parent.def.race.IsMechanoid)
            {
                yield break;
            }
            
            Pawn pawn = (Pawn)parent;
            if (!pawn.IsColonyMech && pawn.skills == null)
                yield break;

            bool isNonViolent = false;

            if(!pawn.IsColonyMech)
                isNonViolent = (pawn.skills.GetSkill(SkillDefOf.Shooting).TotallyDisabled && pawn.skills.GetSkill(SkillDefOf.Melee).TotallyDisabled);
                                ...
                                foreach (var p in Utils.getColonistsAndAllyAndNeutral(pawn.Map).OrderBy(p => p.LabelCap))
                                {
                                    ...
                                    opts.Add(new FloatMenuOption(p.LabelCap, delegate
                                    {
                                        foreach (var entry in Find.Selector.SelectedObjects)
                                        {
                                            ...
                                            if (!cpawn.CanReach(p, PathEndMode.OnCell, Danger.Deadly, false, false, TraverseMode.ByPawn))
                                            {
        ...
        private Gizmo GetDeathSquadForcedTargetGizmo()
        {
            ...
            command_Target.action = delegate (LocalTargetInfo target)
            {
                ...
                foreach (Pawn current in enumerable)
                {
                    ...
                    {
                        c.setDeathSquadForcedTarget(target.Thing);
                    }
                }
            };
