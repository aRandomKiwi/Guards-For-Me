        ...
        public static void FindAttackTarget(Pawn pawn, out IntVec3 selVec, out Thing thing, bool onlyHittableFromPos = false, bool checkAllowedArea = true )
        {
            ...
            for (int i = 0; i < potentialTargetsFor.Count; i++)
            {
                ...
                if ((!attackTarget.ThreatDisabled(pawn) || (Settings.killDownedThreats && p2 != null && p2.Downed)) && AttackTargetFinder.IsAutoTargetable(attackTarget))
                {
                    ...
                    bool cond = ((isMeleeAttack && (!checkAllowedArea || (checkAllowedArea && thing2.Position.InAllowedArea(pawn))) && pawn.CanReach(thing2, PathEndMode.OnCell, Danger.Deadly, false,false, TraverseMode.ByPawn)) || (!isMeleeAttack && (!checkAllowedArea || (checkAllowedArea && ( (thing2.Position.InAllowedArea(pawn) && pawn.CanReach(thing2, PathEndMode.OnCell, Danger.Deadly, false, false, TraverseMode.ByPawn)) || verb.CanHitTarget(thing2))))));
            ...
            if (thing == null && Settings.exterminatorMode)
            {
                ...
                foreach (var cp in pawn.Map.mapPawns.AllPawns)
                {
                    ...
                    bool cond = ((isMeleeAttack && (!checkAllowedArea || (checkAllowedArea && cp.Position.InAllowedArea(pawn))) && pawn.CanReach(cp, PathEndMode.OnCell, Danger.Deadly, false, false, TraverseMode.ByPawn)) || (!isMeleeAttack && (!checkAllowedArea || (checkAllowedArea && ((cp.Position.InAllowedArea(pawn) && pawn.CanReach(cp, PathEndMode.OnCell, Danger.Deadly, false, false, TraverseMode.ByPawn)) || verb.CanHitTarget(cp))))));
        ...
        public static bool enemyInAllowedArea(Pawn pawn)
        {
            ...
            for (int i = 0; i < potentialTargetsFor.Count; i++)
            {
                ...
                if ((!attackTarget.ThreatDisabled(pawn) || (Settings.killDownedThreats && p1 != null && p1.Downed))){
                    ...
                    if ((thing.Position.InAllowedArea(pawn) && pawn.CanReach(thing, PathEndMode.OnCell, Danger.Deadly, false, false, TraverseMode.ByPawn)) || (!isMeleeAttack && verb.CanHitTarget(thing)))
                    {
                        return true;
                    }
                }
            ...
            if (Settings.exterminatorMode)
            {
                foreach (var cp in pawn.Map.mapPawns.AllPawns)
                {
                    ...
                    if ((cp.Position.InAllowedArea(pawn) && pawn.CanReach(cp, PathEndMode.OnCell, Danger.Deadly, false, false, TraverseMode.ByPawn)) || (!isMeleeAttack && verb.CanHitTarget(cp)))
                    {
                        return true;
                    }
                }
