    ...
    class JobGiver_AIGotoPatrolWaypoint : JobGiver_AIFightEnemy
    {
        protected override Job TryGiveJob(Pawn pawn)
        {
            ...
            if(!comp.targetPatrolWP.IsValid)
            {
                ...
                if (!pawn.CanReach(comp.curPatrolWP.Position, PathEndMode.OnCell, Danger.Deadly, false, TraverseMode.ByPawn)
                    || !pawn.Map.pawnDestinationReservationManager.CanReserve(comp.curPatrolWP.Position, pawn))
                {
                    ...
                    while (i != 10)
                    {
                        if (CellFinder.TryFindRandomCellNear(comp.curPatrolWP.Position, pawn.Map, i, (IntVec3 x) => pawn.CanReach(x, PathEndMode.OnCell, Danger.Deadly, false, TraverseMode.ByPawn) && pawn.Map.pawnDestinationReservationManager.CanReserve(x, pawn), out comp.targetPatrolWP))
                            break;
                        i++;
                    }
            ...
            if (enemyTarget != null && !(pawn2 != null && pawn2.IsInvisible()) && verb != null)
            {
